...
执行前首先检测当前项目，并从 `/tmp/fw-flywheel/$PROJECT/` 读取上层传入的上下文：
...
if [ -f "/tmp/fw-flywheel/$PROJECT/ctx-<N>.md" ]; then
  cat "/tmp/fw-flywheel/$PROJECT/ctx-<N>.md"
else
  gh issue view <N>
fi
```
...
**结果文件**（/tmp/fw-flywheel/$PROJECT/result-<N>.md）：
...
cat > "/tmp/fw-flywheel/$PROJECT/result-<N>.md" << EOF
# Issue #<N> 开发结果

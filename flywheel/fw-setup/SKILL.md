...
```text
/fw-setup <repo-name> [--owner <owner>] [--python <3.11|3.12>] [--branch <main|master>] [--dry-run]
/fw-setup --resume              # 从断点恢复
```
...
> 完整内容见源模板 `~/.claude/skills/fw-setup/SKILL.md` 的阶段 3。
...
# 初始化 .claude 目录
mkdir -p .claude/state/fw-inspect .claude/state/fw-audit
...
echo "  /fw-plan <需求描述>"
echo "  /fw-ship <issue-number>"
echo "  /fw-build <issue-number>"
```

## Resume 机制

`/fw-setup --resume` 逐阶段检查已有文件，从第一个缺失的阶段继续。所有阶段独立幂等。

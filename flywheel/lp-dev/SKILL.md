...
执行前首先检测当前项目，并从 `/tmp/lp-flywheel/` 读取上层传入的上下文：
...
```bash
# 读取开发上下文（若存在）
CTX_FILE="/tmp/lp-flywheel/ctx-<N>.md"
if [ -f "$CTX_FILE" ]; then
  echo "[lp-dev] 读取上下文: $CTX_FILE"
  cat "$CTX_FILE"
fi

# 读取 CI 失败日志（--fix 模式，若存在）
CI_FILE="/tmp/lp-flywheel/ci-<mr-number>.md"
if [ -f "$CI_FILE" ]; then
  echo "[lp-dev] 读取 CI 日志: $CI_FILE"
  cat "$CI_FILE"
fi
```

lp-mr 已将 issue 内容、fix_round、前次改动写入 `ctx-<N>.md`，lp-dev 无需自己 `gh issue view`。
...
优先从 `/tmp/lp-flywheel/ctx-<N>.md` 读取（lp-mr 已写入），若无则 fallback：

```bash
cd "$(git rev-parse --show-toplevel)"
if [ -f "/tmp/lp-flywheel/ctx-<N>.md" ]; then
  cat "/tmp/lp-flywheel/ctx-<N>.md"
else
  gh issue view <N>
fi
```
...
BRANCH=$(git symbolic-ref --quiet refs/remotes/origin/HEAD 2>/dev/null | sed 's|.*/||' || echo "master")
git diff "origin/$BRANCH" --stat > /tmp/lp-flywheel/diff-<N>.md
```

```
Agent(subagent_type="claude", prompt="执行 /simplify 对当前改动进行代码审查。diff 见 /tmp/lp-flywheel/diff-<N>.md")
```
...
**结果文件**（/tmp/lp-flywheel/result-<N>.md）：
...
cat > "/tmp/lp-flywheel/result-<N>.md" << EOF
# Issue #<N> 开发结果

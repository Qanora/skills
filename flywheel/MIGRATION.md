...
> **2026-06-19 更新**：新增 `fw-setup` — 全新项目的一键初始化 skill，两个源项目均无此 skill（初始化均靠手动）。
...
| fw-inspect 分析维度 | 网络追踪专用（延迟/跳/健康检查） | 量化筛选专用（因子IC/回测/成本） | **「附录 P」可插拔** |
| fw-build 验证命令 | `ruff check . && ruff format --check .` + `python -m pytest tests/ -v` | 同左 | **「附录：项目特定命令」可插拔** |
| fw-ship 分支基址 | `origin/main` | `origin/master` | **`origin/{DEFAULT_BRANCH}`** |

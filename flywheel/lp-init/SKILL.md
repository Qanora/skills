...
```text
/lp-init <repo-name> [--owner <owner>] [--python <3.11|3.12>] [--branch <main|master>] [--dry-run]
/lp-init --resume              # 从断点恢复
```
...
> 完整内容见源模板 `~/.claude/skills/lp-init/SKILL.md` 的阶段 3。
...
创建 `scripts/` 目录并写入四个脚本：`watch-pr.sh`、`lp-mr-cleanup.sh`、`cleanup-merged-branches.sh`、`commit-msg`。全部执行 `chmod +x scripts/*.sh`。
...
# 初始化 .claude 目录
mkdir -p .claude/state/lp-up .claude/state/lp-dp
...
echo "☐ 脚本: scripts/watch-pr.sh, lp-mr-cleanup.sh, cleanup-merged-branches.sh, commit-msg"
...
echo "  /lp-ms <需求描述>"
echo "  /lp-mr <issue-number>"
echo "  /lp-dev <issue-number>"
```

## Resume 机制

`/lp-init --resume` 逐阶段检查已有文件，从第一个缺失的阶段继续。所有阶段独立幂等。
